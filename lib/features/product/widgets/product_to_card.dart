...
import 'package:meninki/features/product/bloc/product_favorites_cubit/product_favorites_cubit.dart';
            ...
            return Container(
              ...
              child: Row(
                children: [
                  ...
                  Row(
                    children: [
                      BlocBuilder<ProductFavoritesCubit, ProductFavoritesState>(
                        builder: (context, favoriteState) {
                          bool alreadyFavorite = false;
                          if (favoriteState is ProductFavoritesSuccess) {
                            alreadyFavorite = favoriteState.productIds.contains(state.product.id);
                          }
                          return GestureDetector(
                            onTap: () {
                              if (favoriteState is! ProductFavoritesLoading) {
                                context.read<ProductFavoritesCubit>().toggleFavorite(
                                  state.product.id,
                                );
                              }
                            },
                            child: Container(
                              ...
                              child: Center(
                                child:
                                    favoriteState is ProductFavoritesLoading
                      ...
                      BlocBuilder<MyBasketCubit, MyBasketState>(
                        builder: (context, basketState) {
                          ...
                          if (basketState is MyBasketSuccess && selectedComposition != null) {
                            var index = basketState.compositionIds.indexOf(
                              selectedComposition.id ?? 0,
                            );
                            ...
                            onTap: () {
                              ...
                              if (composition == null) {
                                CustomSnackBar.showYellowSnackBar(
                                  context: context,
                                  title: AppLocalizations.of(context)!.selectAllAttributes,
                                );
                                return;
                              }
